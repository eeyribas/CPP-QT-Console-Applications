...
int main(int argc, char *argv[])
{
    ...
    int send_state;
    char sender_buffer_data[] = {'e', 's', 'e', 'n', 'e', 'y', 'r', 'i', 'b', 'a', 's'};
    ...
    socket_bind = bind(tcp_server_socket, (SOCKADDR*)&add_tcp_server, sizeof(add_tcp_server));
    ...
    accept_socket = accept(tcp_server_socket, (SOCKADDR*)&add_tcp_client, &add_tcp_client_state);
    if (accept_socket == INVALID_SOCKET)
        printf("Accept Failed & Error No-> %s\n", WSAGetLastError());
    printf("Connection Accepted.\n");
    sleep(5);

    send_state = send(accept_socket, sender_buffer_data, sender_buffer_length, 0);
    if (send_state == SOCKET_ERROR)
        printf("Sending Failed % Error No-> %s\n", WSAGetLastError());
    printf("Data Sending Success.\n");

    recv_state = recv(accept_socket, recv_buffer_data, recv_buffer_length, 0);
